...
<head>
    ...
    <title>{{ config('app.name', 'Biddie') }}</title>
    ...
    <style>
        body {
            ...
            background: radial-gradient(circle at top, rgba(59, 130, 246, 0.18), transparent 24%),
    ...
    <div class="min-h-screen flex flex-col">
        <header class="relative z-20">
            <div class="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
                <a href="/" class="text-2xl font-bold tracking-tight text-white">Biddie</a>
                <nav class="flex items-center gap-4">
                    <a href="/" class="text-sm font-medium text-slate-200 hover:text-white">Home</a>
                    <a href="{{ route('homepage') }}"
                        class="text-sm font-medium text-slate-200 hover:text-white">Market</a>
                    <a href="{{ route('info') }}" class="text-sm font-medium text-slate-200 hover:text-white">About</a>
                    @auth
                        <a href="{{ route('dashboard') }}"
                            class="text-sm font-medium text-slate-200 hover:text-white">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="text-sm font-medium text-slate-200 hover:text-white">Login</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}"
                                class="text-sm font-medium text-slate-200 hover:text-white">Register</a>
                        @endif
                    @endauth
                </nav>
            </div>
        ...
        <main class="relative z-10 flex-1 overflow-hidden">
            <div
                class="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.24),transparent_25%),radial-gradient(circle_at_bottom_right,_rgba(168,85,247,0.18),transparent_20%)] pointer-events-none">
            </div>

            <section class="relative max-w-7xl mx-auto px-6 py-20 lg:py-28">
                <div class="grid gap-12 lg:grid-cols-[1.25fr_0.95fr] items-center">
                    <div class="hero-card rounded-[2rem] p-10 lg:p-16">
                        <span
                            class="inline-flex items-center rounded-full bg-indigo-500/15 px-4 py-1 text-sm font-semibold text-indigo-200 mb-6">Auction
                            platform for modern buyers & sellers</span>
                        <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-white mb-6">Sell.
                            Bid. Connect. Win.</h1>
                        <p class="max-w-2xl text-lg leading-8 text-slate-300 mb-10">Biddie brings your listings to life
                            with live auctions, activity feeds, social connections, and messaging—so every offer feels
                            fast, fair, and exciting.</p>

                        <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                            <a href="{{ route('homepage') }}"
                                class="cta-button inline-flex items-center justify-center rounded-3xl bg-indigo-500 px-8 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 hover:bg-indigo-400">Enter
                                Marketplace</a>
                            <a href="{{ route('login') }}"
                                class="cta-button inline-flex items-center justify-center rounded-3xl border border-slate-700 bg-slate-900/80 px-8 py-3 text-sm font-semibold text-slate-100 hover:bg-slate-800">Log
                                In</a>
                        </div>
                    </div>

                    <div class="feature-card rounded-[2rem] p-10 border border-white/10 shadow-2xl shadow-slate-900/30">
                        <div class="space-y-8">
                            <div>
                                <h2 class="text-2xl font-semibold text-white mb-3">Why Biddie?</h2>
                                <p class="text-slate-300 leading-7">From curated auctions and live bidding to friend
                                    invites and secure messaging, Biddie is built for community-driven trading.</p>
                            </div>

                            <div class="grid gap-4">
                                <div class="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
                                    <h3 class="text-lg font-semibold text-white">Live Auctions</h3>
                                    <p class="mt-2 text-slate-400">Create and browse active listings with real-time
                                        bidding.</p>
                                </div>
                                <div class="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
                                    <h3 class="text-lg font-semibold text-white">Social Feed</h3>
                                    <p class="mt-2 text-slate-400">Watch community activity and discover trending items.
                                    </p>
                                </div>
                                <div class="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
                                    <h3 class="text-lg font-semibold text-white">Secure Messaging</h3>
                                    <p class="mt-2 text-slate-400">Connect with buyers and sellers instantly.</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <footer class="relative z-20 border-t border-white/10 bg-slate-950/50 py-8">
            <div class="max-w-7xl mx-auto px-6 text-center text-slate-500 text-sm">&copy; {{ date('Y') }} Biddie. Built
                for fast, secure auctions and a smarter marketplace.</div>
        </footer>
    </div>
</body>
...
{{-- Light Mode 12 SVG --}}
<svg class="w-[448px] max-w-none relative -mt-[4.9rem] -ml-8 lg:ml-0 lg:-mt-[6.6rem] dark:hidden" viewBox="0 0 440 376"
